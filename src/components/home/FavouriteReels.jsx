  ...
  return (
    <section className="section favourite-reels-section">
      ...
      <div className="reels-swiper-wrap">
        <Swiper modules={[Autoplay]} loop speed={900} autoplay={{ delay: 2200, disableOnInteraction: false, pauseOnMouseEnter: true }} spaceBetween={14} slidesPerView={2} watchOverflow breakpoints={{ 768: { slidesPerView: 3, spaceBetween: 16 }, 992: { slidesPerView: 4, spaceBetween: 18 }, 1200: { slidesPerView: 6, spaceBetween: 18 } }}>
...
function ReelCard({ reel, product }) {
  ...
  return (
    <article className="reel-card">
      ...
      <button type="button" className="icon-btn reel-control" onClick={toggle} aria-label={playing ? 'Pause reel' : 'Play reel'}>{playing ? <Pause size={17} /> : <Play size={17} />}</button>
      ...
      <button type="button" className="light-btn">Shop Now</button>
    </article>
  );
}
